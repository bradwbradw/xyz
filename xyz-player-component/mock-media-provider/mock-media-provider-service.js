...
/**
 * @ngdoc service
 * @name xyzPlayer.MockMediaProvider
 * @description
 * # MockMediaProvider
 * Service in the xyzPlayer.
 */"use strict";

angular.module("xyzPlayer")
  .factory("MockMediaProvider", function ($q, $log, $window, $timeout, $rootScope, $interval) {
    ...
    var play = function () {
      ...
      var announceDone = function(){
        $log.debug('announcing that the song is done: ',MockMediaProvider.getCurrentSong());
        $rootScope.$emit('mock_song_done');

      };

      MockMediaProvider.songInterval =
        $timeout( announceDone, MockMediaProvider.getCurrentSong().length*1000);


      // non important color changing viz:

      if( MockMediaProvider.animation  &&  MockMediaProvider.animation.cancel){
    ...
    var MockMediaProvider = {
      ...
      getCurrentSong: function(){
        return MockMediaProvider.currentSong;
      },
      position:false,
      status:'initializing',
      animation:{},
