<?php
	if (!class_exists("getID3") && file_exists(dirname(__FILE__)."/getid3/getid3/getid3.php")) { require_once(dirname(__FILE__)."/getid3/getid3/getid3.php"); }
	...
	class mp3Data
	{
		...
		public function getArt($id = 0)
		{
			...
			if (!isset($this->getid3->info["id3v2"]["APIC"][$id]["data"])) { $this->setError("No Attached Picture with ID $id");return FALSE; }
		...
		{
			if (!$this->analyzed) { return $this->notAnalyzed(); }
			if (!isset($this->getid3->info["id3v2"]["APIC"])) { $this->setError("No Attached Pictures");return FALSE; }
			$ra = array();
