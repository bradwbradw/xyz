...
url = song.url;
          if(type == 'file') {
              var smSong = soundManager.createSound({
                 id: 'xyzsong'+song.id,
                 url: MP3_FILE_DIR+song.url,
                onload: function() {
                    console.log('sound loaded!', this.id);

                // container for all sound manager songs

                   }
              });

              smSongs.push(smSong);
              smID = smSongs.length -1;
              song.local = smID;
//              song.title = data.id3_title;
//              song.artist = data.id3_artist;
//              song.image = ;
...
//                    console.log(scData);
              ...
              */
                    ...
                    // these lines have to happen here (scope issues)
                      ...
                      song.artist = scData.user.username;
...
// first, assume these aren't available, then set inside IF blocks
  ...
  /*BASIC*/ video_url = "http://www.youtube.com/v/" + service_id + video_url_suffix ;
...
function loadSong(id,x,y,url,type,status, serviceID, artist, title, image){
...
function loadAll(){
  ...
  $.ajax({
    type: "POST",
//      url: xyz_root_dir+"data/db.php",
      url: db_endpoint,
    data: {action: "loadAll", user: user},
    dataType: "json",
    success: function(loaded){
    dbSongs = loaded[0];
    greatestID = loaded[1];
     /*   $.each(loadedSongs, function(idx, aSong){
          console.log(aSong.url);
          // aSong. stuff
        });*/
    }
  }).done(function( ) {
        console.log('loading songs done - success!');
          ...
          for(var q=0; q<dbSongs.length; q++){
            idid = parseInt(dbSongs[q].id,10);
            if ( idid > greatestID ){
                greatestID = idid;
            }

            loadSong(dbSongs[q].id, dbSongs[q].x, dbSongs[q].y, dbSongs[q].url, dbSongs[q].type, dbSongs[q].status, dbSongs[q].service_id, dbSongs[q].artist, dbSongs[q].title, dbSongs[q].image);
            console.log("type:"+dbSongs[q].type);
          }


          console.log('greatest id is '+ greatestID);
